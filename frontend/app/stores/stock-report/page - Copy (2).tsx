            ...
            <tbody>
              {balance.map((row, i) => (
                <tr key={i} className={`border-b hover:bg-gray-50 ${Number(row.current_qty) <= 0 ? "text-red-400" : ""}`}>
                  ...
                  <td className="px-3 py-2 text-right font-semibold">{Number(row.current_qty).toFixed(2)}</td>
                </tr>
              ))}
              {balance.length > 1 && (
              ...
              <table className="w-full border-collapse text-sm">
                <tbody>
                  <tr className="bg-blue-700 text-white font-bold text-base">
                    <td className="px-14 py-3" colSpan={4}>Grand Total ({deptNames.length} Departments)</td>
                    <td className="px-22 py-3 text-right pl-24">
                      {balance.reduce((s, r) => s + Number(r.opening_qty||0), 0).toFixed(2)}
                    </td>
                    <td className="px-0 py-3 text-right">{grandIn.toFixed(2)}</td>
                    <td className="px-1 py-3 text-right">{grandOut.toFixed(2)}</td>
                    <td className="px-1 py-3 text-right">{grandBal.toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
