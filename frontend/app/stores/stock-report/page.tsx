...
<div className="bg-white rounded-xl shadow overflow-hidden">
  ...
  <table className="table-fixed w-full border-collapse text-sm">
    <thead className="bg-gray-100">
      <tr>
        <th className="border-b px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase w-40">Department</th>
        <th className="border-b px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase w-32">Category Type</th>
        <th className="border-b px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase w-32">Category</th>
    ...
    <tbody>
      {balance.map((row, i) => (
        <tr key={i} className={`border-b hover:bg-gray-50 ${Number(row.current_qty) <= 0 ? "text-red-400" : ""}`}>
          <td className="px-3 py-2 font-medium w-40 truncate whitespace-nowrap overflow-hidden text-ellipsis">{row.dept_name || `Dept ${row.dept_id}`}</td>
          ...
          <td className="px-3 py-2 w-32">{row.category || <span className="text-gray-300">—</span>}</td>
          <td className="px-3 py-2 w-16">{row.unit_of_measure}</td>
          <td className="px-3 py-2 text-right w-24">{Number(row.opening_qty).toFixed(2)}</td>
          <td className="px-3 py-2 text-right w-24 text-green-700">{Number(row.total_in).toFixed(2)}</td>
          <td className="px-3 py-2 text-right w-24 text-orange-600">{Number(row.total_out).toFixed(2)}</td>
          <td className="px-3 py-2 text-right w-24 font-semibold">{Number(row.current_qty).toFixed(2)}</td>
        </tr>
      ))}

      {/* Grand Total — fixed: colSpan=4 covers first 4 cols, then 4 value cells = 8 total */}
      {balance.length > 1 && (
        <tr className="bg-blue-50 font-semibold border-t-2 border-blue-200">
          ...
          <td className="px-3 py-2 text-right text-blue-800">{grandIn.toFixed(2)}</td>
          <td className="px-3 py-2 text-right text-blue-800">{grandOut.toFixed(2)}</td>
          <td className="px-3 py-2 text-right text-blue-800">{grandBal.toFixed(2)}</td>
        </tr>
      )}
    </tbody>
  </table>
              ...
              <div key={deptName} className="bg-white rounded-xl shadow overflow-hidden">
                <div className="bg-gray-700 text-white px-4 py-2 flex justify-between items-center text-sm">
                  ...
                  <span className="text-gray-300 text-xs">
                    In: {deptIn.toFixed(2)} · Out: {deptOut.toFixed(2)} · Balance: {deptBal.toFixed(2)}
                  </span>
                </div>
                  ...
                  <tbody>
                        ...
                        return [
                          ...
                          ...rows.map((row: any, i: number) => (
                            <tr key={`${row.item_id}-${row.dept_id}-${i}`}
                              ...
                              <td className="px-3 py-2 text-right">{Number(row.opening_qty).toFixed(2)}</td>
                              <td className="px-3 py-2 text-right text-green-700">{Number(row.total_in).toFixed(2)}</td>
                              <td className="px-3 py-2 text-right text-orange-600">{Number(row.total_out).toFixed(2)}</td>
                              <td className="px-3 py-2 text-right font-medium">{Number(row.current_qty).toFixed(2)}</td>
                            </tr>
                          )),
                          <tr key={`sub-${catType}-${cat}`} className="bg-yellow-50 text-xs border-b-2 border-yellow-200">
                            ...
                            <td className="px-3 py-1.5 text-right font-semibold text-yellow-800">
                              {rows.reduce((s: number, r: any) => s + Number(r.opening_qty||0), 0).toFixed(2)}
                            </td>
                            <td className="px-3 py-1.5 text-right font-semibold text-yellow-800">{catIn.toFixed(2)}</td>
                            <td className="px-3 py-1.5 text-right font-semibold text-yellow-800">{catOut.toFixed(2)}</td>
                            <td className="px-3 py-1.5 text-right font-semibold text-yellow-800">{catBal.toFixed(2)}</td>
                          </tr>
                        ];
                      })
                    ))}
                    <tr className="bg-gray-200 font-semibold border-t-2 border-gray-400">
                      ...
                      <td className="px-3 py-2 text-right">
                        {deptRows.reduce((s, r) => s + Number(r.opening_qty||0), 0).toFixed(2)}
                      </td>
              ...
              <table className="w-full border-collapse text-sm">
                <tbody>
                  <tr className="bg-blue-700 text-white font-bold text-base">
                    <td className="px-6 py-3" colSpan={5}>Grand Total ({deptNames.length} Departments)</td>
                    <td className="px-0 py-3 text-right pl-28">
                      {balance.reduce((s, r) => s + Number(r.opening_qty||0), 0).toFixed(2)}
                    </td>
