...
export default function DeptMasterPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [dRes, sRes] = await Promise.all([
        fetch(`/api/department/all`,    { credentials: "include" }),
        fetch(`/api/subdepartments/all`, { credentials: "include" }),
      ]);
  ...
  const deleteDept = async (dept: Dept) => {
    ...
    const res = await fetch(`/api/department/${dept.dept_id}`, { method: "DELETE", credentials: "include" });
  ...
  const deleteSubdept = async (sub: Subdept) => {
    ...
    const res = await fetch(`/api/subdepartment/${sub.subdept_id}`, { method: "DELETE", credentials: "include" });
    if (!res.ok) { alert((await res.json()).error || "Failed to delete"); return; }
    await loadAll();
  };
  ...
  const handleEdit = async (deptName: string, editSub: { subdept_id: number; subdept_name: string } | null) => {
    ...
    try {
      ...
      if (deptName !== editDept.dept_name) {
        const res = await fetch(`/api/department/${editDept.dept_id}`, {
      ...
      if (editSub) {
        const res = await fetch(`/api/subdepartment/${editSub.subdept_id}`, {
          method: "PUT", headers: { "Content-Type": "application/json" },
          credentials: "include",
